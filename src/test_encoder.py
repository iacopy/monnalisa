...
@pytest.mark.parametrize('case', [
    {'size': (1, 1), 'n': 1, 'shape': 'e', 'expected': 1 * (32 + (0 + 0) + (0 + 0) * 2 + 1)},
    {'size': (2, 2), 'n': 1, 'shape': 'e', 'expected': 1 * (32 + (1 + 1) + (1 + 1) * 2 + 1)},
    {'size': (2, 2), 'n': 1, 'shape': 't', 'expected': 1 * (32 + (1 + 1) + (1 + 1) * 3 + 1)},
    {'size': (4, 2), 'n': 2, 'shape': 't', 'expected': 2 * (32 + (2 + 1) + (2 + 1) * 3 + 1)},
])
...
def test_decode_rgba():
    ...
    assert actual == {
        ...
        'annotations': {'visibility': [0]},
    }


@pytest.mark.parametrize('im_size,im_mode,n_shapes,shape,expected', [
    [(2, 3), 'L', 2, 'q', 2 * (1 + (1 + 2) + 4 * (1 + 2) + 8)],
    [(2, 3), 'LA', 2, 'q', 2 * (1 + (1 + 2) + 4 * (1 + 2) + 16)],
])
...
@pytest.mark.parametrize('case', [
    {
        'genome': '10000011011111111',
        'shapes': [((255,), [(0, 0), (0, 1), (1, 0)])],
    },
    {
        'genome': '10000011011111110',
        'shapes': [((254,), [(0, 0), (0, 1), (1, 0)])],
    },
    # visibility 0 at pos 8
    {
        'genome': '01100011011111110',
        'shapes': [],
    },
    # All ones
    {
        'genome': '10011111111111111',
        'shapes': [((255,), [(1, 1), (1, 1), (1, 1)])],
    },
    # Extra length
    {
        'genome': '100111111111111110000',
        'shapes': [((255,), [(1, 1), (1, 1), (1, 1)])],
    },
])
